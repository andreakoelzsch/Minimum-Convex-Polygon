...
<p align="center">
  <img src="screenshot_mcp.png" width="600">
</p>
...
The MCPs for each individual are plotted on an OpenStreetMap as the default basemap, individual-specific colors. Below the polygons, the tracks of individuals (with sufficiently long tracks) are displayed in the same matching colors. Users can switch to Topo or Aerial basemaps if desired.
...
The App should work for any kind of (location) data. Each track needs a minimum of 5 locations.
...
This App does not produce Artefacts. The following files can be downloaded manually:

`MCPs_xx_areas.csv`: `xx` representing the chosen percentage. Contains the area in km² of each MPC per track.

`MCPs_xx.html`: `xx` representing the chosen percentage. MCPs polygons on basemap as a HTML file.  

`MCPs_xx.png`: `xx` representing the chosen percentage. MCPs polygons on basemap as a PNG file.
...
### Technical details
The mcp is calculated with the R library `adehabitatHR`. The dataset is reprojected to an Azimuthal Equidistant (AEQD) coordinate system centered on the spatial extent of the data, using meters as units before calculating the mcp, this ensures the area being calcualted correctly.